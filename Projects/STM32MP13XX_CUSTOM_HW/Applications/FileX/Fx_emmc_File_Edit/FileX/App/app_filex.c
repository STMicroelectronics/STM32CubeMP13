...
void fx_thread_entry(ULONG thread_input)
{
  ...
  printf("FileX/LevelX EMMC Application Start.\n");

  if (BSP_MMC_Init(FX_STM32_MMC_INSTANCE) != BSP_ERROR_NONE)
  {
	  /* Indicate that MMC is not initialized properly */
	  BSP_LED_On(LED_RED);
  }
  else
  {
	  if(BSP_MMC_GetCardState(FX_STM32_MMC_INSTANCE) == MMC_TRANSFER_OK)
	  {
  ...
  for( ;; )
  {
    ...
    while(1)
    {
      ...
      if(r_msg == CARD_STATUS_CHANGED)
      {
        ...
        if(BSP_MMC_GetCardState(FX_STM32_MMC_INSTANCE) == MMC_TRANSFER_OK)
        {
          ...
          BSP_LED_Off(LED_RED); /*Led Red Off*/
          break;
        }
        else
        {
          ...
          BSP_LED_Off(LED_GREEN); /*LED Green Off*/
          BSP_LED_On(LED_RED); /*LED Red On*/
        }
      }

    }

    BSP_MMC_DeInit(FX_STM32_MMC_INSTANCE);
    ...
    status =  fx_media_open(&sdio_disk, "STM32_SDIO_DISK", fx_stm32_mmc_driver, (VOID *) 0x00,(VOID *) media_memory, sizeof(media_memory));
    ...
    status =  fx_media_space_available(&sdio_disk, &available_space_pre);

	printf("User available EMMC Flash disk space size before file is written: %lu bytes.\n", available_space_pre);
    ...
    status =  fx_media_space_available(&sdio_disk, &available_space_post);

    printf("User available EMMC Flash disk space size after file is written: %lu bytes.\n", available_space_post);
...
/**
  * @brief  EXTI line detection Falling callback.
