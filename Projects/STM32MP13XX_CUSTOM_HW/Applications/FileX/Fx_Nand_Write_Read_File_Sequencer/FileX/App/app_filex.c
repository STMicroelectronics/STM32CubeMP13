...
static ALIGN_32BYTES (UCHAR media_memory[BYTES_PER_PHYSICAL_PAGE]);

/* Define FileX global data structures.  */
static FX_MEDIA nand_flash_disk;
static FX_FILE fx_file;

/* Define ThreadX global data structures.  */
static TX_THREAD fx_thread;
...
void fx_thread_entry(ULONG thread_input)
{
  ...
  printf("FileX/LevelX NAND-FMC with Sequencer Application Start.\n");

  /* Print the absolute size of the NAND chip*/
  printf("Total NAND Flash Chip size is: %u bytes.\n", NAND_FLASH_SIZE);

  printf("Hold the 'USER' button pressed while booting to format the NAND.\n");

  if (BSP_PB_GetState(BUTTON_USER) == BUTTON_PRESSED)
  {
    printf("Formatting NAND...\n");

    /* Low level NAND formating: mark bad blocks and add metadata for wear leveling */
    status = _lx_nand_flash_format(&nand_flash, "NAND_FLASH", _lx_nand_flash_initialize_driver, fx_lx_nand_driver_buffer, sizeof(fx_lx_nand_driver_buffer));
  ...
  {
    printf("NAND formatting skipped\n");
  }
  ...
  status = fx_file_delete(&nand_flash_disk, TEST_FILENAME);
  /* Check the delete status.  */
  if (status == FX_SUCCESS)
  {
    printf("The file %s was already existing and has been deleted.\n", TEST_FILENAME);
  }
  ...
  /* Create a file called TEST_FILENAME in the root directory.  */
  status =  fx_file_create(&nand_flash_disk, TEST_FILENAME);
  ...
  /* Open the test file.  */
  status =  fx_file_open(&nand_flash_disk, &fx_file, TEST_FILENAME, FX_OPEN_FOR_WRITE);
  ...
  /* Open the test file.  */
  status =  fx_file_open(&nand_flash_disk, &fx_file, TEST_FILENAME, FX_OPEN_FOR_READ);
  /* Check the file open status.  */
  if (status != FX_SUCCESS)
  {
    /* Error opening file, call error handler.  */
  ...
  clusterSize = nand_flash_disk.fx_media_bytes_per_sector * nand_flash_disk.fx_media_sectors_per_cluster;
  printf("The test file occupied a total of %lu cluster(s) (%lu per cluster).\n",
         (available_space_pre - available_space_post) / clusterSize,
         clusterSize);
  ...
  printf("NAND Flash disk tests successful.\n");
