...
UINT  _lx_nand_flash_initialize_driver(LX_NAND_FLASH *nand_flash)
{
  ...
  if (!flash_is_initialized)
  {
    ...
    if (BSP_FMC_NAND_InitWithSequencer(NAND_INSTANCE) != BSP_ERROR_NONE)
#else
...
#if SEQUENCER_ENABLED
  if (BSP_FMC_NAND_ReadWithSequencer(NAND_INSTANCE, &Bsp_Address, (uint8_t *) nand_flash_rw_buffer) != BSP_ERROR_NONE)
  {
    return(LX_ERROR);
  }
  if (BSP_FMC_NAND_WaitSequencerCompletion(NAND_INSTANCE) != BSP_ERROR_NONE)
  {
    return(LX_ERROR);
  }
#else
...
static UINT _lx_nand_flash_pages_read(ULONG block, ULONG page, UCHAR* main_buffer, UCHAR* spare_buffer, ULONG pages)
{
...
#if ECC_ENABLED
#if SEQUENCER_ENABLED
  NAND_EccStatisticsTypeDef statistics;
#else
  ...
  for (pageIndex = 0; pageIndex < pages; pageIndex++)
  {
    ...
    if (main_buffer)
    {
#if ECC_ENABLED
#if SEQUENCER_ENABLED
      if (BSP_FMC_NAND_ReadWithSequencerAndEcc(NAND_INSTANCE, &Bsp_Address, (uint8_t *) main_buffer) != BSP_ERROR_NONE)
      {
        status = LX_ERROR;
        break;
      }
      ...
      BSP_FMC_NAND_GetEccStatistics(NAND_INSTANCE, &statistics);
      if (statistics.BadSectorCount > 0)
#else
...
#else
#if SEQUENCER_ENABLED
      if (BSP_FMC_NAND_ReadWithSequencer(NAND_INSTANCE, &Bsp_Address, (uint8_t *) main_buffer) != BSP_ERROR_NONE)
      {
        status = LX_ERROR;
        break;
      }
...
static UINT _lx_nand_flash_pages_write(ULONG block, ULONG page, UCHAR* main_buffer, UCHAR* spare_buffer, ULONG pages)
{
  ...
  BSP_NAND_AddressTypeDef_t Bsp_Address = {0};
  ULONG pageIndex;
#if ECC_ENABLED && !SEQUENCER_ENABLED
...
#if ECC_ENABLED
#if SEQUENCER_ENABLED
      if (BSP_FMC_NAND_WriteWithSequencerAndEcc(NAND_INSTANCE, &Bsp_Address, (uint8_t *) main_buffer) != BSP_ERROR_NONE)
      {
        status = LX_ERROR;
        break;
      }
...
#else
#if SEQUENCER_ENABLED
      if (BSP_FMC_NAND_WriteWithSequencer(NAND_INSTANCE, &Bsp_Address, (uint8_t *) main_buffer) != BSP_ERROR_NONE)
      {
        status = LX_ERROR;
        break;
      }
