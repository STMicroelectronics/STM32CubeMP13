/**
  ...
  *          MX25L51245G XSPI-NOR memory of STM32MP13XX_VALID BOARD.
  *           
  ******************************************************************************
  * @attention
  *
...
int Write_XSPI(uint8_t* pData, uint32_t Address, uint32_t Size)
{
  ...
  if (BSP_XSPI_WriteMemory((uint8_t*) pData, (Address & (0x0fffffff)), Size) != HAL_OK) {
      HAL_SuspendTick();
      return LOADER_FAIL;
  }
...
int Read_XSPI(uint8_t* pData, uint32_t Address, uint32_t Size)
{
  ...
  if (BSP_XSPI_Read((uint8_t*) pData, Address, Size) != HAL_OK) {
      HAL_SuspendTick();
      return LOADER_FAIL;
  }
