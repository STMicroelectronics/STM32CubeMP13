/**
  ******************************************************************************
  * @file    loader_XSPI_NOR.c
  * @author  MCD Application Team
  * @brief   This file defines the loader operations for external
  *          MX25L51245G XSPI-NOR memory of STM32MP13XX_CUSTOM_HW BOARD.
  *
  ******************************************************************************
  * @attention
  *
 ...
 * @retval  LOADER_FAIL = 0 : Operation failed
 */
int Write_XSPI(uint8_t* buffer, uint32_t Address, uint32_t Size)
{
 ...
 * @retval  LOADER_FAIL = 0 : Operation failed
 */
int Read_XSPI(uint8_t* buffer, uint32_t Address, uint32_t Size)
{
  ...
  if (BSP_XSPI_Read((uint8_t*) buffer, Address, Size) != HAL_OK) {
      HAL_SuspendTick();
      return LOADER_FAIL;
  }
 ...
 * Optional command - delete in case usage of mass erase is not planned
 * Inputs    :
 *      none
 * outputs   :
 *     none
