...
/**
  ******************************************************************************
  * @file    FSBLA/FSBLA_eMMC/Src/main.c
...
typedef void (*nullFunctPtr)(void);
...
uint8_t aRxBuffer[MMC_BLOCKSIZE * NB_BLOCK_HEADER];
uint8_t aGPTBuffer[MMC_BLOCKSIZE * NB_BLOCK_GPT];
...
int main(void)
{
  ...
  /* Unsecure SDMMC1. */
  LL_ETZPC_Set_SDMMC2_PeriphProtection(ETZPC, LL_ETZPC_PERIPH_PROTECTION_READ_WRITE_NONSECURE);
  ...
  *p=DDR_PATTERN;
  ...
  /*##- Read GPT Table ####################*/
  if(HAL_MMC_ReadBlocks(&MMCHandle, aGPTBuffer, OFFSET_HEADER_GPT, NB_BLOCK_GPT, 3000) != HAL_OK)
  {
    Error_Handler();
  }
  ...
  if(HAL_MMC_ReadBlocks_DMA(&MMCHandle, aRxBuffer, exampleOffset, NB_BLOCK_HEADER) != HAL_OK)
  {
    Error_Handler();
  }
  ...
  for (int i = 1;i <= NB_HEXA_NB_FOR_IMAGE_LENGTH; i++)
  {
    ...
    if(LITLE_ENDIAN == 1)
    {
      /*Litle Endian : The most significant byte of the data is placed at the byte with the lowest address */
  ...
  nbCubeExampleBlock = sizeCubeExample / MMC_BLOCKSIZE;
  ...
  if(HAL_MMC_ReadBlocks_DMA(&MMCHandle, (uint8_t *)DDR_MEM_ADD, (exampleOffset + NB_BLOCK_HEADER), nbCubeExampleBlock+1) != HAL_OK)
  {
    Error_Handler();
  }
  ...
  //uint32_t *ptr=DDR_MEM_ADD;
  p_AppEntryPoint = (nullFunctPtr)DDR_MEM_ADD;
  p_AppEntryPoint();
