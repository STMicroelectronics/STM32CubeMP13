...
#define SD_BLOCKADDRESS        ((uint32_t)0)         /* SD Card Block Address to write/read data */
#define BLOCK_COUNT	(0x00760000 - 1)
...
int main(void)
{
  ...
  /* ENABLE Boot from BOOT1 partition with ACK enable */
  HAL_MMC_ConfigBootPartition(&hmmc2, (BOOT_FROM_BOOT1_PARTITION | BOOT_ACK_ENABLE));

  /* Erase first 2 MB */
  Erase(SD_BLOCKADDRESS, BLOCK_COUNT);
  MassErase();
...
int Init(void)
{
	  /*##-2- Initialize MMC instance #####################*/
	  /* Initialize all configured peripherals */
	  MX_SDMMC2_MMC_Init();

	  /* Try to switch to High Speed Mode , if supported by the card */
	  HAL_MMC_ConfigSpeedBusOperation(&hmmc2, SDMMC_SPEED_MODE_HIGH);

	  HAL_MMC_ConfigBootPartition(&hmmc2, (BOOT_FROM_USER_PARTITION | BOOT_ACK_ENABLE));
...
void MassErase(void)
{
    if(HAL_MMC_Erase(&hmmc2, 0, 1) != HAL_OK)
    {
      Error_Handler();
	}
...
void Erase(uint32_t block_start_addr, uint32_t block_end_addr)
{
//    if(HAL_MMC_Erase(&hmmc2,block_start_addr,block_end_addr) != HAL_OK)
//    {
//      Error_Handler();
//	}
//    /* Wait for the Card Transfer status */
...
void Write(uint32_t BlockAdd, uint32_t tot_blocks, uint8_t *buffer)
{
	if(HAL_MMC_WriteBlocks(&hmmc2, buffer, BlockAdd, tot_blocks, SDMMC_DATATIMEOUT)!= HAL_OK)
    {
        Error_Handler();
	}
	/* Wait for the Card Transfer status */
...
void Read(uint32_t BlockAdd, uint32_t tot_blocks, uint8_t *buffer)
{
	...
	if(HAL_MMC_ReadBlocks(&hmmc2, buffer, BlockAdd, tot_blocks, SDMMC_DATATIMEOUT)!= HAL_OK)
	{
		Error_Handler();
	}
	/* Wait for the Card Transfer status */
...
void configuration(void *ptr)
{
	...
	if (NULL != conf)
	{
		switch(conf->command)
		{
		case CONFIGURE_DISABLE_BOOT1_PARTITION:
			/* This will enable Boot from BOOT1 Partition and Enable ACK */
			HAL_MMC_ConfigBootPartition(&hmmc2, (BOOT_FROM_BOOT1_PARTITION | BOOT_ACK_ENABLE));
			break;
		case CONFIGURE_ENABLE_BOOT1_PARTITION:
			/* This will enable Boot from BOOT1 Partition and Enable ACK as well as enable RW access to BOOT1 partition */
			HAL_MMC_ConfigBootPartition(&hmmc2, (BOOT_ACK_ENABLE | BOOT_FROM_BOOT1_PARTITION | BOOT1_PARTITION_RW_ACCESS));
			break;
		default:
			/* This will enable Boot from USER Partition and Enable ACK */
			HAL_MMC_ConfigBootPartition(&hmmc2, (BOOT_ACK_ENABLE | BOOT_FROM_USER_PARTITION));
			break;
		}
	}
}
