...
void HAL_ADC_MspInit(ADC_HandleTypeDef *hadc)
{
  ...
  /* Unsecure SYSRAM so that DMA2 (which is always non-secure) can access it. */
  LL_ETZPC_SetSecureSysRamSize(ETZPC, 0);

  /* Unsecure ADC2 so that DMA2 (which is always non-secure) can access it. */
  LL_ETZPC_Set_ADC2_PeriphProtection(ETZPC, LL_ETZPC_PERIPH_PROTECTION_READ_WRITE_NONSECURE);

  /* Unsecure GPIO */
  ADCx_CHANNELa_GPIO_PORT->SECCFGR = 0;
