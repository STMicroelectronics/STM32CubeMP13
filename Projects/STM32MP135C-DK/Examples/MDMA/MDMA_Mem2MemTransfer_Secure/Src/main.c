...
static void MX_MDMA_Init(void)
{
  ...
  /* Secure SYSRAM, the secured MDMA channel can access it. */
  LL_ETZPC_SetSecureSysRamSize(ETZPC, 0xFFFFFFFF);

  /* Secure SRAM3, the secured MDMA channel can access it. */
  LL_ETZPC_Set_SRAM3_PeriphProtection(ETZPC, LL_ETZPC_PERIPH_PROTECTION_READ_WRITE_SECURE);

  /* Set the parameters to be configured */
  hmdma_memtomem.Init.Request = MDMA_REQUEST_SW;
  /* Configure MDMA in SECURE Mode */
  hmdma_memtomem.Init.SecureMode = MDMA_SECURE_MODE_ENABLE;
