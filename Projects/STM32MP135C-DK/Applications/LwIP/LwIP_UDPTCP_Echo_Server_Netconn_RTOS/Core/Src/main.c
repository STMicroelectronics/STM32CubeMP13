...
int main(void)
{
  ...
  printf("\rLwIP_UDPTCP_Echo_Server_Netconn_RTOS Example started\r\n");
...
static void StartThread(void * argument)
{
  ...
  while (1)
  {
  	if ((eth_conf_s[0].DHCP_state == DHCP_ADDRESS_ASSIGNED) && ((eth_conf_s[1].DHCP_state == DHCP_ADDRESS_ASSIGNED)))
  	{
  		break;
  	}
  	else
