...
void lp_enter(void)
{
  ...
  /* Save zdatas */
  HAL_RTCEx_BKUPWrite(&RTCHandle_BKUP, RTC_BKP_DR3,(uint32_t) (zdatas));

  /* Prepare low power entry - Standby */
  BSP_PMIC_PrepareStandbySelfRefresh();

  /* Enable DDR retention in Standby */
  PWR->CR3 |= (0x1 << 10);
  
  /* Setup POPL for minimal duration in Standby */
  PWR->CR3 |= (0x1F << 17);

  /* Make sure the wakeup buttons flags are cleared */
  PWR->WKUPCR |= (0x3F);
...
void lp_exit(void)
{
  RTC_HandleTypeDef RTCHandle_BKUP = {0};

  zdatas = HAL_RTCEx_BKUPRead(&RTCHandle_BKUP, RTC_BKP_DR3);

  hddr.wakeup_from_standby = true;
  hddr.self_refresh = true;
  hddr.zdata = zdatas;
  ...
  /* Return to DDR - address provided by application is in backup register 0 */
...
int main(void)
{
  ...
  if (HAL_RTCEx_BKUPRead(&RTCHandle_BKUP, RTC_BKP_DR0) == 0) {
  /* Configure PMIC */
  ...
  /* Check if this is a wakeup or a power on */
  if (HAL_RTCEx_BKUPRead(&RTCHandle_BKUP, RTC_BKP_DR0) != 0)
    lp_exit();
  ...
  *p=DDR_PATTERN;
  ...
  if (HAL_RTC_Init(&RTCHandle_BKUP) != HAL_OK)
  {
    Error_Handler();
  }
  
  /* Save return address in backup register 2 for user app */
