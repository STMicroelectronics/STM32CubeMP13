...
int main(void)
{
  ...
  printf("\n\r Hello World - Landed in user application in DDR\n\r");
...
// add your config test
  ...
  /* Show that the application in DDR is running by toggling red & blue LEDS */
  while (count < 100)
  {
    /* USER CODE END WHILE */
  ...
  /* Program a wakeup source - push button of DK */
  HAL_PWR_EnableWakeUpPinIT(PWR_WAKEUP_PIN1);
  HAL_PWR_EnableWakeUpPin(PWR_WAKEUP_PIN1_LOW);
  IRQ_Enable(MPU_WAKEUP_PIN_IRQn);

  /* Call FSBL-A by retrieving its address in backup register 2 */
