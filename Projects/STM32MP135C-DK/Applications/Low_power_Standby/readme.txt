/**
  ...
  * @file    Low_power_Standby/readme.txt
  * @author  MCD Application Team
  * @brief   Description of the Standby application.
  ******************************************************************************
  * @attention
  *
...
This application shows a Standby entry with DDR in self refresh to keep its content.
The Cortex A7 and the VDDCORE are OFF (0V).
At wakeup the bootrom directly retrieves the FSBL-A from the SD card to execute the wakeup treatments.
The DDR application is still present and is not reloaded. When the FSBL-A has treated the wakeup, it
jumps back into the DDR application.
The wakeup can be triggered by pressing B3 (wake_up button)

This application relies on two signed firmwares.
The FSBL-A is located in sysram and is loaded from SD card by the bootrom at power on and at each wakeup from
Standby.
...
- Both LEDs are off in Standby meaning there is no software running anymore.
...
@par Directory contents
  ...
  - Low_power_Standby/BSP_BasicTemplates_low_power                The files that are needed to build the DDR application
  - Low_power_Standby/FSBLA_Standby                               The files that are needed to build the FSBL-A
...
In order to make the program work, you must do the following :
 - Import the Low_power_Standby folder, it will open both projects
