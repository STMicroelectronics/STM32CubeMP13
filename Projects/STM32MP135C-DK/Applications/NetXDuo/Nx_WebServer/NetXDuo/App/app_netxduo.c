...
static TX_THREAD AppServerThread;
...
static NX_PACKET_POOL EthPool;
static NX_PACKET_POOL WebServerPool;
static NX_PACKET_POOL DhcpPool;
...
/* The SD card buffer must be 4-byte aligned for the SDMMC1 DMA and it is put in the .SdBufferSection linker section which must be in a non-cacheable area
 * to avoid cache maintenance. */
static uint8_t SdBuffer[SD_BUFFER_SIZE] __attribute__ ((aligned(4), section(".SdBufferSection")));
...
UINT MX_NetXDuo_Init(VOID *memory_ptr)
{
  ...
  printf("\n\rNx_Webserver application started..\n\r");
 ...
 ret = nx_web_http_server_create(&HTTPServer, "WEB HTTP Server", &IpInstance, CONNECTION_PORT,&SDMedia, pointer,
  ...
  if (ret != TX_SUCCESS)
  {
    ...
    /* Allocate the memory for toggle blue led thread  */
  ...
  /* Allocate the server packet pool. */
  ret = tx_byte_allocate(byte_pool, (VOID **) &pointer, DHCP_POOL_SIZE, TX_NO_WAIT);

  /* Check server packet pool memory allocation. */
  if (ret != NX_SUCCESS)
  {
    printf("DHCP packed pool memory allocation failed : 0x%02x\n\r", ret);
    Error_Handler();
  }

  /* Create the server packet pool. */
  ret = nx_packet_pool_create(&DhcpPool, "DHCP Packet Pool", DHCP_PACKET_SIZE, pointer, DHCP_POOL_SIZE);

  /* Check for server pool creation status. */
  if (ret != NX_SUCCESS)
  {
    printf("DHCP packet pool creation failed : 0x%02x\n\r", ret);
    Error_Handler();
  }

  ret = nx_dhcp_packet_pool_set(&DHCPClient, &DhcpPool);
  if (ret != NX_SUCCESS)
  {
    printf("DHCP packet pool setup failed : 0x%02x\n\r", ret);
  }
...
UINT webserver_request_notify_callback(NX_WEB_HTTP_SERVER *server_ptr, UINT request_type, CHAR *resource, NX_PACKET *packet_ptr)
{
  ...
  /*
  * At each new request we toggle the blue led, but in a real use case this callback can serve
...
void nx_server_thread_entry(ULONG thread_input)
{
  ...
  status = fx_media_open(&SDMedia, "STM32_SDIO_DISK", fx_stm32_sd_driver, SD_DRIVER_INFO_POINTER, SdBuffer, SD_BUFFER_SIZE);
