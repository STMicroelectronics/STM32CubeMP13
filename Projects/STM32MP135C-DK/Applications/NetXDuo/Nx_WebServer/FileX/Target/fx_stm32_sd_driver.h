...
static inline void clean_invalidate_cache(uint32_t start, uint32_t size)
{
  UNUSED(start);
  UNUSED(size);
}
...
static inline void clean_invalidate_cache(uint32_t start, uint32_t size)
{
  ...
  while (current < end)
  {
    L1C_CleanInvalidateDCacheMVA((void*)current);
    current += 32U;
  }
}
...
/* We must use clean_invalidate_cache() because buffers address and size are not always aligned on 32 bytes boundaries.
 * Cache invalidation would destroy valid data before or after the read buffer. So the clean before invalidation is necessary. */
#define invalidate_cache_by_addr(__ptr__, __size__) clean_invalidate_cache((uint32_t)(__ptr__), (uint32_t)(__size__))
