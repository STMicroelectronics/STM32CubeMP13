...
int main(void)
{
  ...
  /* SYSRAM is set as non-secure because we use it for SDMMC1 DMA transfers which cannot transfer to secured memories. */
...
/**
  * @brief Configures EXTI Line 4 (connected to PH.4 pin) in interrupt mode
  * @param None
  * @retval None
  */
static void EXTI4_IRQHandler_Config(void)
{
  /* Set configuration except Interrupt and Event mask of Exti line 4*/
  EXTI_ConfigStructure.Line = EXTI_LINE_4;
  EXTI_ConfigStructure.Trigger = EXTI_TRIGGER_RISING_FALLING;
  EXTI_ConfigStructure.GPIOSel = EXTI_GPIOH;
  EXTI_ConfigStructure.Mode = EXTI_MODE_INTERRUPT;

  HAL_EXTI_SetConfigLine(&hexti, &EXTI_ConfigStructure);

  /* Enable and set line 4 Interrupt to priority 14*/
  IRQ_SetPriority(EXTI4_IRQn, 14);
