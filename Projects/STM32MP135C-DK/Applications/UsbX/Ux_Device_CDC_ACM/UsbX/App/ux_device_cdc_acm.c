...
void CDC_Init_HS(void *cdc_acm)
{
  ...
  /* Get uart4 baudrate */
  CDC_VCP_LineCoding.ux_slave_class_cdc_acm_parameter_baudrate = huart4.Init.BaudRate;

  /*set the data type : only 8bits and 9bits are supported */
  switch (huart4.Init.WordLength)
  {
  ...
  /* Get uart4 Parity */
  CDC_VCP_LineCoding.ux_slave_class_cdc_acm_parameter_parity = huart4.Init.Parity;

  /* Get uart4 StopBits */
  CDC_VCP_LineCoding.ux_slave_class_cdc_acm_parameter_stop_bit = huart4.Init.StopBits;
  ...
  /* Any data received will be stored in "UserTxBufferHS" buffer  */
  if (HAL_UART_Receive_IT(&huart4, (uint8_t *)UserTxBufferHS, 1) != HAL_OK)
  {
...
/**
  ...
  */
void CDC_DeInit_HS(void *cdc_acm)
{
  /* USER CODE BEGIN 4 */
  ...
  while (1)
  {
    ...
    if (device->ux_slave_device_state == UX_DEVICE_CONFIGURED)
    {
      ...
      if (ux_status == UX_SUCCESS)
      {
        ...
        ux_device_class_cdc_acm_read(cdc_acm, (UCHAR *)UserRxBufferHS, 64,
                                     &actual_length);
        if (actual_length != 0)
        {

          #if defined(CACHE_USE)
          /* Invalidate cache for DMA push */
            L1C_CleanInvalidateDCacheAll();
          #endif
          /* Send the data via UART */
          if (HAL_UART_Transmit_DMA(&huart4, (uint8_t *)UserRxBufferHS,
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
  ...
  if (HAL_UART_Receive_IT(&huart4, (uint8_t *)UserTxBufferHS + UserTxBufPtrIn, 1) != HAL_OK)
  {
    /* Transfer error in reception process */
    Error_Handler();
  }
                                ...
                                *CDC_VCP_LineCoding)
{
  ...
  huart4.Init.BaudRate = CDC_VCP_LineCoding->ux_slave_class_cdc_acm_parameter_baudrate;
  ...
  HAL_UART_Receive_IT(&huart4, (uint8_t *)(UserTxBufferHS + UserTxBufPtrIn), 1);
}
/* USER CODE END 0 */
