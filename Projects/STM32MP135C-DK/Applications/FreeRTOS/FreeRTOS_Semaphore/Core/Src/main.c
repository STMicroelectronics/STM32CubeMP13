...
/* USER CODE BEGIN PV */
xTaskHandle SEM_Task1Handle = NULL;
xTaskHandle SEM_Task2Handle = NULL;
xSemaphoreHandle semaphoreHandle;


__IO uint32_t SemStatus = 0;
/* USER CODE END PV */
...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
static void SemaphoreTask1(void *param);
static void SemaphoreTask2(void *param);


#define semaphoreTask1_PRIORITY (8)
#define semaphoreTask2_PRIORITY (1)
...
int main(void)
{
  ...
  xTaskCreate(SemaphoreTask1,"semaphoreTask1", 4096, semaphoreHandle, semaphoreTask1_PRIORITY, &SEM_Task1Handle);
  if (SEM_Task1Handle == NULL)
  {

  		Error_Handler();
  }

  xTaskCreate(SemaphoreTask2,"semaphoreTask2", 4096, semaphoreHandle, semaphoreTask2_PRIORITY, &SEM_Task2Handle);
  if (SEM_Task2Handle == NULL)
  {
...
/* USER CODE BEGIN Header_SemaphoreTask1 */
/**
  * @brief  Function implementing the SemaphoreTask1 task.
  * @param  argument: Not used
  * @retval None
  */
/* USER CODE END Header_SemaphoreTask1 */
static void SemaphoreTask1(void *param)
{

    /* USER CODE BEGIN 5 */
    uint32_t count = 0;
    xSemaphoreHandle semaphore = (xSemaphoreHandle) param;
    ...
    for (;;)
    {
	   if (semaphore != NULL)
	   {
		   SemStatus =  xSemaphoreTake(semaphore, portMAX_DELAY);
		 /* Try to obtain the semaphore */
		 if (SemStatus == pdPASS)
		 {
		   count = xTaskGetTickCount() + 5000;

		   /* Toggle LED7 every 200 ms for 5 seconds */
		   ...
		   SemStatus = xSemaphoreGive(semaphore);

		   /* Suspend ourseleves to execute task 2 (lower priority)  */
		   vTaskSuspend(SEM_Task1Handle);
		 }
      }
...
/**
* @brief Function implementing the SemaphoreTask2 task.
* @param argument: Not used
* @retval None
*/
...
static void SemaphoreTask2(void *param)
{
	  ...
	  for (;;)
	  {
	    if (semaphore != NULL)
	    {
	      ...
	      if (SemStatus == pdPASS)
	      {
	        /* Resume Task 1 (higher priority)*/
	       vTaskResume(SEM_Task1Handle);
